...
class Table
{
	...
	private function __cell(&$html, $value, $column, $row_data)
	{
		...
		// apply cell template
		if(isset($this->__templates[$column]) || array_key_exists($column, $this->__templates))
		{
	...
	private function __row(&$html)
	{
		if(!$this->__is_row) // start row
		{
			...
			$html .= '<tr' . $this->__getAttributes('tr', $this->__index_row) . '>'
				. $this->end_of_line;
			$this->__is_row = true;
		}
		else // end row
	...
	public function get()
	{
		...
		if(count($this->__data) > 0)
		{
			...
			$html['head'] = '<table' . (isset($this->__attributes['table'])
				&& is_array($this->__attributes['table'])
				? self::__attributes($this->__attributes['table']) : '' ) . '>'
				. $this->end_of_line;
			$html['body'] = '';
			...
			{
				$html['head'] .= '<thead>' . $this->end_of_line . '<tr'
					. $this->__getAttributes('head', 0, false) . '>' . $this->end_of_line;
				$i = 0;
